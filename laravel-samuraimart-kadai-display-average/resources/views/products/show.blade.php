    ...
    <div class="col">
      <div class="d-flex flex-column">
        ...
        <div class="d-flex align-items-center mb-3">
                <div class="star-rating d-flex" data-rate="{{ round($product->reviews->avg('score') * 2) / 2 }}"></div><div class="average-review-rate d-flex ms-1">{{ round($product->reviews->avg('score'), 1) }}</div>
              </div>
        <p class="">
          {{ $product->description }}
        </p>
        <hr>
        <p class="d-flex align-atems-end">
        ￥{{ $product->price }}(税込)
        </p>
        <hr>
      </div>
      @auth
      <form method="POST" action="{{ route('carts.store') }}" class="m-3 align-atems-end">
        @csrf
